...
class Experts(nn.Module):
    ...
    #     # print(type(self.experts[0]), sum(x.numel() for x in self.experts[0].parameters()))
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            for expert in self.experts:
                out = expert(x)
                outs.append({"last": out["last"].detach()})
                if self.skip_shape is not None:
                    outs[-1].update({"skip": out["skip"].detach()})

        last = [out["last"] for out in outs]
        last = torch.stack(last, 1)
        ret = {"last": last}
        if self.skip_shape is not None:
            skip = [out["skip"] for out in outs]
            skip = torch.stack(skip, 1)
            ret.update({"skip": skip})
