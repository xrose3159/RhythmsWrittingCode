...
class TextBox:
    def __init__(self, w, h, x, y, font=None, callback=None):
        ...
        self.__surface.fill((255, 255, 255))  # 填充白色背景
        # 如果font为None,那么效果可能不太好，建议传入font，更好调节
        if font is None:
            self.font = pygame.font.SysFont('Microsoft YaHei', 35)
        else:
            self.font = font
    ...
    def draw(self, dest_surf):
        ...
        center_x = self.x + (self.width - text_width) // 2
        center_y = self.y + (self.height - text_height) // 2
        # 绘制背景色
        ...
        # 绘制居中的文字
        dest_surf.blit(text_surf, (center_x, center_y))
        # 绘制联想词
        if self.state == 1:
            ...
            #                (0, 0, self.width, self.height)
            #                )
            dest_surf.blit(self.word_list_surf,(center_x - 50, center_y - 50))
